...
function dumpDexAt(dexObjPtr, tag) {
  try {
    ...
    const size = dexObjPtr.add(32).readU32();
    if (size < 112 || size > 100 * 1024 * 1024) {
      console.log("[" + tag + "] bad size " + size);
      return;
    }
    const begin = ptr("0x" + beginQ.toString(16));
    let fsize = -1;
    try {
      fsize = begin.add(32).readU32();
    } catch (e) {
      console.log("[" + tag + "] unreadable begin " + begin);
      return;
    }
    if (fsize !== size) {
      console.log("[" + tag + "] fsize mismatch " + fsize + " != " + size);
      return;
    }
    ...
    send({ t: "dex", idx: dumped, size, tag }, bytes);
  } catch (e) {
...
Java.perform(function() {
  ...
  loaders.forEach(function(loader, li) {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      let s;
      try {
        s = Arrays.toString.overload("[J").call(Arrays, cookie);
      } catch (e) {
        console.log("[L" + li + " el" + i + "] cookieErr " + e);
        continue;
      }
      ...
      for (let j = 0; j < nums.length; j++) {
        ...
        try {
          ...
          console.log("[L" + li + "e" + i + "c" + j + "] parse " + e);
        }
      }
    }
  });
