...
var APPLY_SCRIPT = [
  ...
  "  else",
  "    maxid=$((maxid + 1))",
  '    grep -v \'</settings>\' "$TMP" > "$NEW"',
  '    echo "    <setting id=\\"$maxid\\" name=\\"$uid\\" value=\\"$val\\" package=\\"$pkg\\" defaultValue=\\"$val\\" defaultSysSet=\\"false\\" tag=\\"null\\" />" >> "$NEW"',
  "    echo '</settings>' >> \"$NEW\"",
  '    mv "$NEW" "$TMP"',
  "  fi",
  'done < "$LIST"',
  "",
  'if [ "$plain" = 1 ]; then',
  '  cat "$TMP" > "$SSAID" || exit 1',
  "else",
  '  xml2abx "$TMP" "$SSAID" || exit 1',
  "fi",
